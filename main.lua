...
-- Filtro Rígido de Materiais Aceitos (Estritamente em Minúsculo) - Atualizado apenas com os existentes
...
-- Sistema de Antivoid / Fly Temporário para a Coleta
    ...
    for _, obj in ipairs(scraps:GetChildren()) do
        if itemEstaAtivoNoMundo(obj) then
            if not obj:FindFirstChild("Nunes_ESP") then
                ...
                elseif nomeL == "emerald" then corMaterial = Color3.fromRGB(0, 255, 100)  -- Verde Esmeralda
                elseif nomeL == "meat" then corMaterial = Color3.fromRGB(255, 100, 100)     -- Rosa/Vermelho Carne
                end
...
-- =============================================================================
-- SISTEMA DE COLETA RESILIENTE (COM ANTIVOID / FLY SEGURO)
-- =============================================================================
...
local function executarColetaMateriais(nomeItem)
    ...
    logarAcao("Fila de Coleta", "Iniciando recolhimento de " .. total .. " " .. nomeItem .. "(s). Ativando Fly...", 3)
    ativarFlyTemporario(hrp) -- Ativa o Fly aqui para não cair no void
    ...
    for idx, obj in ipairs(alvos) do
        if itemEstaAtivoNoMundo(obj) then
            ...
            if parte then
                hrp.CFrame = parte.CFrame + Vector3.new(0, 1.5, 0) -- Fica ligeiramente acima do item
                task.wait(0.3)
    ...
    desativarFlyTemporario() -- Desativa o Fly de segurança ao retornar
...
TabColeta:CreateButton({ Name = "Teleportar e Coletar Todas as Esmeraldas (Emerald)", Callback = function() executarColetaMateriais("Emerald") end })
TabColeta:CreateButton({ Name = "Teleportar e Coletar Todas as Carnes (Meat)", Callback = function() executarColetaMateriais("Meat") end })
...
TabVenda:CreateButton({
    Name = "Vender Itens (Gold, Diamond, Copper, Emerald, Meat)",
    Callback = function()
        local hrp = getHRP()
        local char = LocalPlayer.Character
        local mochila = LocalPlayer:FindFirstChild("Backpack")
